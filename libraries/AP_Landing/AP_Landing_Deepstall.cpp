...
void AP_Landing_Deepstall::do_land(const AP_Mission::Mission_Command& cmd, const float relative_altitude)
{
    ...
    if (!landing_point.compField1.flags.relative_alt && !landing_point.compField1.flags.terrain_alt) {
        ...
        const bool is_armed, const bool is_flying, const bool rangefinder_state_in_range)
{
    ...
    case DEEPSTALL_STAGE_ESTIMATE_WIND:
        {
        landing.nav_controller->update_loiter(landing_point, landing.aparm.loiter_radius, landing_point.compField1.flags.loiter_ccw ? -1 : 1);
        ...
        delta *= (landing_point.compField1.flags.loiter_ccw ? -1 : 1);
    ...
    case DEEPSTALL_STAGE_ARC:
        ...
        if (!landing.nav_controller->reached_loiter_target() ||
            ...
            landing.nav_controller->update_loiter(arc, landing.aparm.loiter_radius, landing_point.compField1.flags.loiter_ccw ? -1 : 1);
            return false;
        }
...
void AP_Landing_Deepstall::build_approach_path(bool use_current_heading)
{
    ...
    float arc_heading_deg = target_heading_deg + (landing_point.compField1.flags.loiter_ccw ? -90.0f : 90.0f);
