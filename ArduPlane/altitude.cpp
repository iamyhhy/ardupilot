...
void Plane::set_target_altitude_location(const Location &loc)
{
    ...
    if (loc.compField1.flags.relative_alt) {
     ...
     */
    float height;
    if (loc.compField1.flags.terrain_alt && terrain.height_above_terrain(height, true)) {
        target_altitude.terrain_following = true;
...
void Plane::set_offset_altitude_location(const Location &loc)
{
     ...
     */
    float height;
    if (loc.compField1.flags.terrain_alt && 
...
float Plane::height_above_target(void)
{
    ...
    if (!next_WP_loc.compField1.flags.relative_alt) {
    ...
    // also record the terrain altitude if possible
    float terrain_altitude;
    if (next_WP_loc.compField1.flags.terrain_alt && 
