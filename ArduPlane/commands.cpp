...
void Plane::set_next_WP(const struct Location &loc)
{
    ...
    if (next_WP_loc.lat == 0 && next_WP_loc.lng == 0) {
        ...
        if (next_WP_loc.alt == 0) {
            ...
            next_WP_loc.compField1.flags.relative_alt = false;
            next_WP_loc.compField1.flags.terrain_alt = false;
        }
    }
...
{
    if (aparm.loiter_radius < 0 || guided_WP_loc.compField1.flags.loiter_ccw) {
        loiter.direction = -1;
    } else {
        loiter.direction = 1;
    }
