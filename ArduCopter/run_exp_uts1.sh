...
# this sscript is to:
# 1. run the michigan school bus - commutor north mission
...
do
    ...
    #run the michigan school bus-commutor north mission from the beginning:
    # start ardupilot
    ...
    python ./ardu_ctrl/run_random_stop_uts1.py >& ./ardu_ctrl/uts1/run_1_$i.txt &
    sleep 1500
    ...
    then
        ...
        #interception point
        gps="$(python ./ardu_ctrl/run_restart_from_intercept_uts1.py 1),103" 
    
        echo $gps
        cp $data_file ./ardu_ctrl/uts1/serialize_data/"$i"_$data_file
    

        echo _"$gps"_

        #gnome-terminal -e "bash -c \"./run.sh base_i $gps\"">& ./ardu_ctrl/baseline1/sim_log_"$i".txt &
        gnome-terminal -e "bash -c \"./run.sh com_i $gps\"" &
        sleep 20

        echo "new simulation has started..."
        
        #deserialize and start the simulator
        python ./ardu_ctrl/run_after_intercept_uts1.py >& ./ardu_ctrl/uts1/run_2_$i.txt &
        ...
        echo -e "fail\nserialized.data not found">./ardu_ctrl/uts1/run_2_$i.txt
        for j in `ps -aux | grep -i ardu | awk '{print $2}'`; do kill -9 $j; done
        for j in `ps -aux | grep -i mavpro | awk '{print $2}'`; do kill -9 $j; done
        for j in `ps -aux | grep -i sim_vehicle | awk '{print $2}'`; do kill -9 $j; done
        for j in `ps -aux | grep -i run_random_ | awk '{print $2}'`; do kill -9 $j; done
